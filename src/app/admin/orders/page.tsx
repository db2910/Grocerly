...
export default function OrdersPage() {
  const { orders, updateOrderStatus, refreshData, loading } = useAdminStore();
      ...
      <div className="flex flex-col md:flex-row justify-between items-start md:items-center gap-4 mb-8">
        <div>
          ...
          <p className="text-slate-500 font-medium mt-1">
            {orders.length} total orders • {totalRevenue.toLocaleString()} RWF total revenue
          </p>
        </div>
        <button 
          onClick={() => refreshData()}
                      ...
                      <div className="flex justify-end gap-2">
                        {order.status === "pending" && (
                          <button onClick={() => updateOrderStatus(order.id, "confirmed")}
                            className="px-3 py-1.5 bg-blue-50 text-blue-600 hover:bg-blue-100 rounded-lg text-xs font-bold transition-colors shadow-sm">
                            Confirm
                          </button>
                        )}
                        {order.status === "confirmed" && (
                          <button onClick={() => updateOrderStatus(order.id, "delivered")}
                            className="px-3 py-1.5 bg-emerald-50 text-emerald-600 hover:bg-emerald-100 rounded-lg text-xs font-bold transition-colors shadow-sm">
                            Deliver
                          </button>
                        )}
                        {["pending", "confirmed"].includes(order.status) && (
                          <button onClick={() => updateOrderStatus(order.id, "cancelled")}
                            className="px-3 py-1.5 bg-red-50 text-red-500 hover:bg-red-100 rounded-lg text-xs font-bold transition-colors shadow-sm">
                            Cancel
                          </button>
                        )}
                      </div>
